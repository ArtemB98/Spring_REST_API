...
public interface EmployeesRepository extends JpaRepository<Employees, Long> {
   ...
   // @Query("select e1.lastName from Employees e1 where e1.departmentsByDepartmentId in (select e2.departmentsByDepartmentId from Employees e2 where e1.employeeId <> e2.employeeId and e1.departmentsByDepartmentId = e2.departmentsByDepartmentId and e1.salary < e2.salary and e1.hireDate < e2.hireDate)")
  ...
  //  @Query("select e1 from Employees e1 where e1.salary >(select avg(e2.salary) from Employees e2 where e1.departmentsByDepartmentId = e2.departmentsByDepartmentId) order by e1.salary")
